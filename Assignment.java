...
public class Assignment {
	...
	public static boolean isQueenComplete(CSP c) {
		
		//if the row is still -1. not complete
	...
	public static boolean ConsistentQueen(CSP c, int i, int row) {//c, position, domain
		c.variables.get(i).row=row;//set assignment to this value (domain)
		boolean consistent1 = isConsistentNQueens1(c);
		boolean consistent2 = isConsistentNQueens2(c);
		if(consistent1==false) {
			return false;
		}
		if(consistent2==false) {
	...
	//check consistency for nQueens
	public static boolean isConsistentNQueens1(CSP c) {
		ArrayList<Variables> list = c.variables;
		for(int first = 0; first < list.size() - 1; first++) {
			for(int second = first + 1; second < list.size(); second++) {
				
				if(list.get(first).row == -1 || list.get(second).row == -1) {
					return true;
				}
				
				if(list.get(first).row == list.get(second).row) {
					return false;
				}
			}
		}
	...
	public static boolean isConsistentNQueens2(CSP c) {
		ArrayList<Variables> list = c.variables;
		for(int first = 0; first < list.size() - 1; first++) {
			for(int second = first + 1; second < list.size(); second++) {
				...
				if(Math.abs(list.get(first).row - list.get(first).col) == Math.abs(list.get(second).row - list.get(second).col)) {
					return false;
				}
			}
		}
